...
class JupyterhubOps:
    ...
    def install(self):
        ...
        try:
            # Run `apt-get update`
            apt.update()
            apt.add_package(["python3.12-venv", "nodejs", "npm"])
        ...
        # Install additional dependencies pre.
        self._install_extra_deps_pre()
    ...
    def uninstall(self):
        ...
        cmd = ["npm", "uninstall", "-g", "configurable-http-proxy"]
        logger.debug(f"## Uninstalling extra dependencies: {cmd}")
        try:
    ...
    def _install_extra_deps_pre(self):
